...
def test_viewset():
    ...
    data = {'pid': 'GOBUS18d45e5f99', 'secret': bus_constants.MYBOOKING_TOKEN}
    ...
    assert response.data['data'][0]['booking_id'] == 'GOBUS18d45e5f99'
...
def test_viewset_client_user():
    client = APIClient()
    url = '/apis/v1/my-bookings/'
    data = dict(page_size=2, page=1, user_id=13, all=1, secret=bus_constants.MYBOOKING_TOKEN)
...
def test_viewset_client_user_not_all():
    client = APIClient()
    url = '/apis/v1/my-bookings/'
    data = dict(page_size=2, page=1, user_id=13, secret=bus_constants.MYBOOKING_TOKEN)
    response = client.get(url, data)
    import pdb;pdb.set_trace()
    assert response.status_code == 200
    assert response.data['count'] == 3
...
def test_viewset_client_user_not_all_failure():
    client = APIClient()
    url = '/apis/v1/my-bookings/'
    data = dict(page_size=5, page=2, user_id=18, secret=bus_constants.MYBOOKING_TOKEN)
    response = client.get(url, data)
    import pdb;pdb.set_trace()
