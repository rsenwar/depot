...
def test_viewset_client_user():
    ...
    data = dict(page_size=2, page=1, u_id=13, all=1, secret=bus_constants.MYBOOKING_TOKEN)
...
def test_viewset_client_user_not_all():
    client = APIClient()
    url = '/apis/v1/my-bookings/'
    data = dict(page_size=2, page=1, u_id=13, secret=bus_constants.MYBOOKING_TOKEN)
    response = client.get(url, data)
    import pdb;pdb.set_trace()
    assert response.status_code == 200
    assert response.data['count'] == 3
